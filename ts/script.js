...
var Cement = (function (_super) {
    __extends(Cement, _super);
    function Cement(id, name, price, description, inStock, list) {
        var _this = _super.call(this, id, name, price, description, inStock) || this;
        _this.id = id;
        _this.name = name;
        _this.price = price;
    ...
    Cement.prototype.Init = function () {
        var obj = _super.prototype.Init.call(this);
        if (this.isSoft) {
            var p = document.createElement("p");
            p.setAttribute("class", "card-text text-info m-0");
            p.innerHTML = "Есть 25 кг";
            obj.firstChild.firstChild.insertBefore(p, obj.firstChild.firstChild.childNodes[2]);
        }
        if (this.haveColors.length > 0) {
            var p = document.createElement("p");
        ...
        if (document.getElementById('isSoft') == null && this.isSoft != null && this.isSoft) {
            ...
            lab.appendChild(inp);
            lab.innerHTML += "Только 25 кг<br>";
            var div = document.getElementById('myTools');
            div.appendChild(lab);
        }
        this.Embed(obj);
    };
    Cement.prototype.CalculateFlags = function () {
        this.isSoft = false;
        if (this.list != null)
            for (var i = 0; i < this.list.length; i++)
...
function CheckSoft(flag) {
    ...
    if (flag) {
        for (var i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Cement && productList[i].isSoft)
                productList[i].Init();
    }
    else {
        for (var i = 0; i < this.productList.length; i++)
...
var Speakers = (function (_super) {
    ...
    function Speakers(id, name, price, description, inStock, list) {
        var _this = _super.call(this, id, name, price, description, inStock) || this;
        _this.id = id;
        _this.name = name;
        _this.price = price;
    ...
    Speakers.prototype.Init = function () {
        ...
        if (document.getElementById('isPortable') == null && this.isPortable != null && this.isPortable) {
            var inp = document.createElement("input");
            inp.setAttribute("type", "checkbox");
            ...
            var lab = document.createElement("p");
            lab.appendChild(inp);
            lab.innerHTML += "Только переносной<br>";
            var div = document.getElementById('myTools');
            div.appendChild(lab);
        }
        this.Embed(obj);
    };
    Speakers.prototype.CalculateFlags = function () {
...
function CheckPortable(flag) {
    ...
    if (flag) {
        for (var i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Speakers && productList[i].isPortable)
                productList[i].Init();
    }
    else {
        for (var i = 0; i < this.productList.length; i++)
...
var productList = [
    ...
    new Cement(8, "Цемент", 666, "Прекрасный выбор! Качественный материал", 90, [
        { isSoft: true, color: Color.Pink, quantity: 1 },
        { isSoft: false, color: Color.Pink, quantity: 1 }
    ]),
    new Speakers(8, "Видеомагнитофон", 666, "Отличный выбор! Качественный товар", 90, [
        { isPortable: true, color: Color.Pink, quantity: 1 },
        { isPortable: false, color: Color.Pink, quantity: 1 }
    ]),
    new Balalaika(9, "Балалайка2", 217, "Обычная балалайка белорусской фирмы Змрочныя мелодыі.", 1)
];

...
class Cement extends Product {
    isSoft: boolean; //Есть большие размеры
    haveColors: string[]; //Цвета которые есть
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public list?:Cement[]) {
        super(id, name, price, description, inStock); 
        this.CalculateFlags();
        this.Init();
    }
...
function CheckSoft(flag: boolean) {
    ...
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Cement && (<Cement>productList[i]).isSoft) (<Cement>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
class Speakers extends Product {
    isPortable: boolean; //Есть большие размеры
    haveColors: string[]; //Цвета которые есть
...
function CheckPortable(flag: boolean) {
    ...
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Speakers && (<Speakers>productList[i]).isPortable) (<Speakers>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
let productList: Product[] = [
    ...
    new Cement(8, "Цемент", 666, "Отличный выбор, качественный материал", 90, 
        [
            {isSoft: true, color: Color.Pink, quantity: 1},
            {isSoft: false, color: Color.Pink, quantity: 1}
        ]),
    new Speakers(8, "Видеомагнитофон", 666, "Это прекрасный выбор, не пожалеете!", 90, 
        [
